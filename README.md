...
v1.5:
基本按照ecs架构：（并且要注意性能）
main.py：一开始进去什么都没有,用于界面（world）的切换
world.py：管理system，entity，以及entity的component
entity.py：实体类：允许增加删除组件，查找组件。用于创造初始化各种单位地图啥的。。
component.py：各种组件类：只保存数据，包含camera.py：照相机组件, 含各种全局设置，静态数学函数，控制台
input.py：输入系统
render.py：渲染系统 （含ui绘制）
turn.py：单人（偏向模拟经营，打ai，类似矮人要塞/rimworld/mc）多人（类似回合制的魔兽争霸，兵棋，文明6，维多利亚3），能像sabaki跳转至任何节点
system.py: 移动系统，战斗系统，地图系统
实现鼠标移动至屏幕边缘自动移动地图
